...
answer_check_model = genai.GenerativeModel("gemini-flash-latest")
...
def check_answer(clue: str, answer: str, response: str) -> list[bool, str]:
    ...
    """
    response = answer_check_model.generate_content(
        _JEOPARDY_PROMPT.format(clue=clue, question=answer, response=response)
    ).text
